...
def paf(path, start, end, min_aligned, min_identity=.90, min_mapq=20):
...
def main():
    ...
    p.add_argument("--min-aligned-bp",type=int,default=300,
                   help="minimum aligned bases for a contig alignment to count as island evidence; "
                        "keep at or above the assembler contig-length floor "
                        "(params.targeted_min_contig_len), below which MEGAHIT cannot contribute "
                        "and assembler_agreement reports artefactual discordance")
    a=p.parse_args(); ev=rows(a.read_evidence)[0]; length=a.island_end-a.island_start
    hits={"megahit":paf(a.megahit_paf,a.region_start,a.region_end,a.min_aligned_bp),"metaspades":paf(a.metaspades_paf,a.region_start,a.region_end,a.min_aligned_bp)}
    ...
    result=dict(ev,megahit_reference_covered_bp=bp["megahit"],megahit_reference_coverage=f'{cov["megahit"]:.6f}',megahit_supporting_contigs=len(best["megahit"]),metaspades_reference_covered_bp=bp["metaspades"],metaspades_reference_coverage=f'{cov["metaspades"]:.6f}',metaspades_supporting_contigs=len(best["metaspades"]),assembler_agreement=agreement,final_structural_evidence=call)
    ...
    for assembler,color,label in (("megahit","#277DA1","B  MEGAHIT"),("metaspades","#F8961E","C  metaSPAdes")):
        svg.append(f'<text x="{L}" y="{y}" class="label">{label}: {len(best[assembler])} canonical supporting contigs; {100*cov[assembler]:.1f}% island coverage</text>'); y+=18
        if not best[assembler]: svg.append(f'<text x="{L+20}" y="{y+12}" class="sub">No contig passes ≥{a.min_aligned_bp} bp, ≥90% identity, MAPQ ≥20</text>'); y+=35
        else:
